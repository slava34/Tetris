...
export class Tetris {
    ...
    generatePlayfield() {
        this.playfield = new Array(PLAYFIELD_ROWS).fill()
            .map(() => new Array(PLAYFIELD_COLOMNS).fill(0));
    }
    ...
    generateTetromino() {
        ...
        const column = PLAYFIELD_COLOMNS / 2 - Math.floor(matrix.length / 2);
        const row = -2;
    ...
    isOutsideOfGameBoard(row, column) {
        return this.tetromino.column + column < 0 ||
            this.tetromino.column + column >= PLAYFIELD_COLOMNS ||
    ...
    dropRowsAbove(rowToDelete) {
        ...
        this.playfield[0] = new Array(PLAYFIELD_COLOMNS).fill(0);
    }
